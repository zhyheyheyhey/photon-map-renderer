...
void SceneParser::parsePerspectiveCamera() {
    ...
    int height = readInt();
    getToken(token);
    double focal_distance = -1., radius = 1.;
    if (!strcmp(token, "focal")) {
        focal_distance = readdouble();
        getToken(token);
    }
    ...
    assert (!strcmp(token, "}"));
    camera = new PerspectiveCamera(center, direction, up, width, height, angle_radians, focal_distance, radius);
}
...
void SceneParser::parseLights() {
    ...
    while (true) {
        ...
        if (strcmp(token, "DirectionalLight") == 0) {
            lights[num_lights++] = parseDirectionalLight();
        } else if (strcmp(token, "PointLight") == 0) {
            lights[num_lights++] = parsePointLight();
        } else {
            return ;
...
Material *SceneParser::parseMaterial() {
    ...
    filename[0] = 0;
    Vector3f diffuseColor(0.9, 0.9, 0.9), emissionColor(0, 0, 0);
    int refl = 0;
    getToken(token);
    assert (!strcmp(token, "{"));
    while (true) {
    ...
    auto *answer = new Material(diffuseColor, emissionColor, refl);
    return answer;
}
...
Plane *SceneParser::parsePlane() {
    ...
    if (!strcmp(token, "image")) {
        ...
        assert (!strcmp(token, "}"));
        assert (current_material != nullptr);
        return new Plane(normal, offset, current_material, img, axis_x, axis_y, offset_x, offset_y);
    }
    else {
        assert (!strcmp(token, "}"));
        assert (current_material != nullptr);
...
Triangle *SceneParser::parseTriangle() {
    ...
    assert (!strcmp(token, "}"));
    assert (current_material != nullptr);
    if (current_material -> emissionColor != Vector3f::ZERO) {
        TriangleLight* light = new TriangleLight(v0, v1, v2, current_material -> emissionColor);
