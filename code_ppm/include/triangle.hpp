...
public:
	...
	bool is_inside( const Vector3f& point) {
		...
		if(Vector3f().dot(normal1, normal2) < 1e-50)
			return false;
		if(Vector3f().dot(normal3, normal2) < 1e-50)
			return false;
		return true;
	}

	bool intersect( const Ray& ray,  Hit& hit , double tmin) override {
        double t_intersect = -(d + Vector3f().dot(normal, ray.getOrigin())) / Vector3f().dot(normal, ray.getDirection());
